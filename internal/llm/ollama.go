...
func convertToOllamaTools(tools []Tool) api.Tools {
	...
	for i, tool := range tools {
		...
		required := make([]string, len(tool.Function.Parameters["required"].([]string)))
		for i, v := range tool.Function.Parameters["required"].([]string) {
			required[i] = v
		}
		...
		for propName, propValue := range rawProps {
			...
			var d string
			if v, ok := propMap["description"]; ok {
				d = v.(string)
			}
