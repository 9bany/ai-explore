...
func createMemoryAgent() *swarm.Agent {
	agent := swarm.NewAgent("MemoryAgent", "llama3.1", llm.Ollama)
	...
	agent.Functions = []swarm.AgentFunction{
		{
			Name:        "store_fact",
			...
			Function: func(args map[string]interface{}, contextVars map[string]interface{}) swarm.Result {
		...
		},
		{
			...
			Function: func(args map[string]interface{}, contextVars map[string]interface{}) swarm.Result {
				...
				return swarm.Result{Data: result}
			},
		},
	}
...
func main() {
	...
	client := swarm.NewSwarm(apiKey, llm.Ollama)
