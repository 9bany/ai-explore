...
func printStepResult(step swarm.StepResult) {
...
`
	...
	workflow := swarm.NewWorkflow(apiKey, llm.Ollama, swarm.CollaborativeWorkflow)

	// Set up cycle handling
	workflow.SetCycleHandling(swarm.ContinueOnCycle)
...
Make direct changes to the content, don't delegate tasks.`,
	...
	workflow.AddAgentToTeam(editor, swarm.DocumentTeam)
	workflow.AddAgentToTeam(reviewer, swarm.DocumentTeam)
	workflow.AddAgentToTeam(writer, swarm.DocumentTeam)

	// Set editor as document team leader
	if err := workflow.SetTeamLeader(editor.Name, swarm.DocumentTeam); err != nil {
