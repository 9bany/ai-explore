		...
		Functions: []swarm.AgentFunction{
			{
				...
				Function: func(args map[string]interface{}, contextVariables map[string]interface{}) swarm.Result {
					...
					case "divide":
						if y == 0 {
							return swarm.Result{Success: false, Data: "Error: division by zero"}
						}
						result = x / y
					default:
						return swarm.Result{Success: false, Data: fmt.Sprintf("Error: unknown operation: %s", op)}
					}

					if err != nil {
						return swarm.Result{Success: false, Data: fmt.Sprintf("Error: %v", err)}
					}

					return swarm.Result{Success: true, Data: fmt.Sprintf("%.2f", result)}
				},
			},
		},
	}
